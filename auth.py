...
class Delegate(DefaultDelegate):
    def __init__(self, device):
        ...
        self.device = device
        self.pkg = 0
    # Allows the reception of notifications and the exchange of keys
    def handleNotification(self, hnd, data):
        ...
        # The fetch characteristic controls the communication with the activity characteristic.
            ...
            if data[:3] == b'\x10\x01\x01':
                ...
                log.info("Fetch data from {}-{}-{} {}:{}".format(year, month, day, hour, minute))
                ...
                if self.device.last_timestamp > self.device.end_timestamp - timedelta(minutes=1):
                    ...
                    return
                log.info("Trigger more communication")
                time.sleep(1)
                t = self.device.last_timestamp + timedelta(minutes=1)
                self.device.start_get_previews_data(t)

            elif data[:3] == b'\x10\x02\x04':
                log.warning("No more activity fetch possible")
                return
            else:
                log.error("Unexpected data on handle " + str(hnd) + ": " + str(data))
                return
...
class miband3_auth(Peripheral):
    ...
    def __init__(self, mac_address, timeout=0.5, debug=False):
        ...
        self._log.info('Mi Band 3 - Connecting to ' + mac_address)
        Peripheral.__init__(self, mac_address, addrType=ADDR_TYPE_RANDOM)
        self._log.info('Mi Band 3 - Connected')
    ...
    def _auth_notif(self, enabled):
        if enabled:
            self._log.info("Mi Band 3 - Enabling Auth Service notifications status...")
            self._desc_auth.write(b"\x01\x00", True)
        elif not enabled:
            self._log.info("Mi Band 3 - Disabling Auth Service notifications status...")
            self._desc_auth.write(b"\x00\x00", True)
        else:
            self._log.error("Mi Band 3 - Something went wrong while changing the Auth Service notifications status...")
    ...
    # Send encrypted key
    def _send_key(self):
        self._log.info("Mi Band 3 - Sending Key...")
        self._char_auth.write(self._send_key_cmd)
        self.waitForNotifications(self.timeout)

    # Asks for the pairing key
    def _req_rdn(self):
        self._log.info("Mi Band 3 - Requesting random number...")
        self._char_auth.write(self._send_rnd_cmd)
        self.waitForNotifications(self.timeout)

    # Send the pairing key encrypted
    def _send_enc_rdn(self, data):
        self._log.info("Mi Band 3 - Sending encrypted random number")
        cmd = self._send_enc_key + self._encrypt(data)
        send_cmd = struct.pack('<18s', cmd)
        self._char_auth.write(send_cmd)
        self.waitForNotifications(self.timeout)

    # Convert received bytes to interpretable information
    # Data parsers:
    ...
    def _parse_date(self, bytes):
        ...
        # Responsible for calculating leap years:
        fractions256 = struct.unpack('b', bytes[8])[0] if len(bytes) >= 9 else None
        date_info = {"date": datetime(*(year, month, day, hours, minutes, seconds)), "day_of_week": day_of_week, "fractions256": fractions256}
    ...
    # Queue to receive heart rate and acceleration data
    ...
    def initialize(self):
        ...
        while True:
            ...
            if self.state == AUTH_STATES.AUTH_OK:
                self._log.info('Mi Band 3 - Initialized')
    ...
    def authenticate(self):
        ...
        while True:
            self.waitForNotifications(0.1)
            if self.state == AUTH_STATES.AUTH_OK:
                self._log.info('Mi Band 3 - Authenticated')
                return True
            elif self.state is None:
                continue
    ...
    def send_custom_alert(self, type):
        if type == 5:
            ...
            base_value = '\x04\x01'
                ...
                base_value = '\x03\x01'
        phone = input('Mi Band 3 - Sender Name or Caller ID')
    ...
    def change_date(self):
        log.info('Mi Band 3 - Change date and time')
        ...
        input('Mi Band 3 - Date Changed, press any key to continue')

    def dfuUpdate(self, fileName):
        log.info('Update Firmware/Resource')
        ...
        crc &= 0xFFFF
        log.info('Mi Band 3 - CRC Value is--> {}'.format(crc))
        input('Mi Band 3 - Press Enter to Continue')
        ...
        with open(fileName) as f:
          while True:
            c = f.read(20)
            if not c:
              log.info("Mi Band 3 - Update Over")
              break
            log.info('Mi Band 3 - Writing Resource %s' % c.encode('hex'))
            char1.write(c)
        # after update is done send these values
        char.write(b'\x00', withResponse=True)
        self.waitForNotifications(0.5)
        log.info('Mi Band 3 - CheckSum is --> {}{}{}'.format(hex(crc & 0xFF), hex((crc >> 8) & 0xFF)))
        ...
        log.warning('Mi Band 3 - Update Complete')
        input('Mi Band 3 - Press Enter to Continue')
    ...
    def start_get_previews_data(self, start_timestamp):
            ...
            self.waitForNotifications(0.1)
            log.info("Mi Band 3 - Trigger activity communication")
...
class miband4_auth(Peripheral):
    ...
    def __init__(self, mac_address, key=None, timeout=0.5, debug=False):
        ...
        logging.basicConfig(format=FORMAT)
        self._log = log
        self._log.info('Mi Band 4 - Connecting to ' + mac_address)
        Peripheral.__init__(self, mac_address, addrType=ADDR_TYPE_PUBLIC)
        self._log.info('Mi Band 4 - Connected')
        if not key:
    ...
    # Send encrypted key
    def _send_key(self):
        self._log.info("Mi Band 4 - Sending Key...")
    ...
    def _auth_notif(self, enabled):
        if enabled:
            self._log.info("Mi Band 4 - Enabling Auth Service notifications status...")
            self._desc_auth.write(b"\x01\x00", True)
        elif not enabled:
            self._log.info("Mi Band 4 - Disabling Auth Service notifications status...")
            self._desc_auth.write(b"\x00\x00", True)
        else:
            self._log.error("Mi Band 4 - Something went wrong while changing the Auth Service notifications status...")

    # Notifications that require authentication
    def _auth_previews_data_notif(self, enabled):
        if enabled:
            self._log.info("Mi Band 4 - Enabling Fetch Char notifications status...")
            self._desc_fetch.write(b"\x01\x00", True)
            self._log.info("Mi Band 4 - Enabling Activity Char notifications status...")
            self._desc_activity.write(b"\x01\x00", True)
            self.activity_notif_enabled = True
        else:
            self._log.info("Mi Band 4 - Disabling Fetch Char notifications status...")
            self._desc_fetch.write(b"\x00\x00", True)
            self._log.info("Mi Band 4 - Disabling Activity Char notifications status...")
    ...
    def initialize(self):
        ...
        while True:
            self.waitForNotifications(0.1)
            if self.state == AUTH_STATES.AUTH_OK:
                self._log.info('Mi Band 4 - Initialized')
                self._auth_notif(False)
                return True
            elif self.state is None:
                continue
    ...
    # Asks for the pairing key
    def _req_rdn(self):
        self._log.info("Mi Band 4 - Requesting random number...")
        self._char_auth.write(self._send_rnd_cmd)
        self.waitForNotifications(self.timeout)

    # Send the pairing key encrypted
    def _send_enc_rdn(self, data):
        self._log.info("Mi Band 4 - Sending encrypted random number")
        cmd = self._send_enc_key + self._encrypt(data)
        send_cmd = struct.pack('<18s', cmd)
        self._char_auth.write(send_cmd)
        self.waitForNotifications(self.timeout)
    ...
    def dfuUpdate(self, fileName):
        log.info('Mi Band 4 - Update Firmware/Resource')
        svc = self.getServiceByUUID(UUIDS.SERVICE_DFU_FIRMWARE)
        char = svc.getCharacteristics(UUIDS.CHARACTERISTIC_DFU_FIRMWARE)[0]
        extension = os.path.splitext(fileName)[1][1:]
        fileSize = os.path.getsize(fileName)
        ...
        with open(fileName) as f:
            while True:
                ...
                crc ^= ((crc & 0xFF) << 5) & 0xFFFFFF
        crc &= 0xFFFF
        log.info('Mi Band 4 - CRC Value is--> %s' % crc)
        ...
        with open(fileName) as f:
            while True:
                ...
                if not c:
                    log.info("Mi Band 4 - Update Over")
                    break
                log.info('Mi Band 4 - Writing Resource', c.encode('hex'))
                char1.write(c)
        # after update is done send these values
        char.write(b'\x00', withResponse=True)
        self.waitForNotifications(0.5)
        log.info('Mi Band 4 - CheckSum is --> {}{}{}'.format(hex(crc & 0xFF), hex((crc >> 8) & 0xFF)))
        checkSum = b'\x04' + bytes(chr(crc & 0xFF), 'utf-8') + bytes(chr((crc >> 8) & 0xFF), 'utf-8')
        char.write(checkSum, withResponse=True)
        if extension.lower() == "fw":
            self.waitForNotifications(0.5)
            char.write(b'\x05', withResponse=True)
        log.warning('Mi Band 4 - Update Complete')
        input('Mi Band 4 - Press Enter to Continue')
    ...
    def start_get_previews_data(self, start_timestamp):
        if not self.activity_notif_enabled:
            self._auth_previews_data_notif(True)
            self.waitForNotifications(0.1)
        log.info("Mi Band 4 - Trigger activity communication")
        year = struct.pack("<H", start_timestamp.year)
